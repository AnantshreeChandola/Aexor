...
def with_db_error_handling(func: Callable[..., T]) -> Callable[..., T]:
    ...
    async def wrapper(*args, **kwargs) -> T:
        ...
        try:
            ...
            raise DatabaseIntegrityError(f"Database integrity constraint violated: {e!s}")
            ...
            raise DatabaseConnectionError(f"Database operation failed: {e!s}")
...
def with_user_existence_check(user_check_query: str | None = None):
    """
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        async def wrapper(*args, **kwargs) -> T:
            ...
            try:
                ...
                result = await session.execute(text(user_check_query), {"user_id": user_id})
                if not result.fetchone():
                    raise UserNotFoundError(user_id)

            except SQLAlchemyError as e:
                logger.error(f"User existence check failed in {func.__name__}: {e}")
                raise DatabaseConnectionError(f"Failed to verify user existence: {e!s}")
...
async def execute_with_retry(
    operation: Callable[[], T], max_retries: int = 3, retry_delay: float = 1.0
) -> T:
    """
