    ...
    def handle_user_not_found(error, user_id=None) -> JSONResponse:
        ...
        return JSONResponse(
            ...
            content=ErrorResponse(
                error_code="USER_NOT_FOUND", message=str(error), details={"user_id": str(user_id)}
            ).model_dump(),
        )
...
def create_error_handler_middleware():
    ...
    async def error_handler_middleware(request, call_next):
        try:
            ...
            from shared.database.error_handler import DatabaseError, UserNotFoundError
            ...
            try:
                ...
                if error_code:
                    return JSONResponse(
                        ...
                        content=ErrorResponse(error_code=error_code, message=str(e)).model_dump(),
                    )
                else:
                    return APIErrorHandler.handle_generic_error(e)
