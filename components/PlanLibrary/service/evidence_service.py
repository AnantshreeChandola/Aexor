...
class EvidenceService:
    ...
    def similarity_to_evidence_item(
        self,
        plan_id: str,
        intent_type: str,
        ...
        Confidence = success_rate * similarity_score for similarity results.

        Args:
            plan_id: ULID plan identifier
            intent_type: Plan intent type
            similarity_score: Cosine similarity score
            success_rate: Plan success rate
            pattern_summary: Summary of plan pattern

        Returns:
            EvidenceItem with combined confidence
        """
        confidence = min(success_rate * similarity_score, 1.0)

        return EvidenceItem(
            type="plan",
            key=f"{intent_type}_similar_{plan_id[:8]}",
            value={
                "intent": intent_type,
                "success_rate": round(success_rate, 3),
