...
class TestRedisSessionStore:
    ...
    async def test_redis_session_store_save_and_get(self, store, mock_redis_client, sample_session):
        ...
        result = await store.get(sample_session.user_id, sample_session.session_id)
        assert result is not None
        assert result.session_id == sample_session.session_id

    async def test_redis_session_store_get_missing(self, store, mock_redis_client):
        """Returns None for non-existent key."""
        mock_redis_client.get.return_value = None
        result = await store.get("user_1", "ses_nonexistent")
        assert result is None

    async def test_redis_session_store_delete_existing(self, store, mock_redis_client):
        """Returns True when key deleted."""
        mock_redis_client.delete.return_value = 1
        assert await store.delete("user_1", "ses_abc") is True

    async def test_redis_session_store_delete_missing(self, store, mock_redis_client):
        """Returns False when key did not exist."""
        mock_redis_client.delete.return_value = 0
        assert await store.delete("user_1", "ses_nope") is False

    async def test_redis_session_store_ttl_refresh(self, store, mock_redis_client, sample_session):
        """Verify SETEX called with 3600 TTL."""
        await store.save(sample_session)
        args = mock_redis_client.setex.call_args
        assert args[0][1] == 3600

    async def test_redis_session_store_error_wrapping(self, store, mock_redis_client):
        """RedisError wrapped as SessionStoreUnavailableError."""
        mock_redis_client.get.side_effect = redis.exceptions.RedisError("conn refused")
...
class TestLLMBasedParser:
    ...
    async def test_llm_parser_single_message(self, parser, mock_llm_adapter):
        """Returns ParseResult with intent + entities."""
        mock_llm_adapter.generate.return_value = (
            '{"intent": "schedule_meeting", "entities": {"attendee": "Alice"}, "constraints": {}}'
        )
        result = await parser.parse("Meet with Alice")
        assert result.intent == "schedule_meeting"
        assert result.entities["attendee"] == "Alice"

    async def test_llm_parser_with_context(self, parser, mock_llm_adapter, sample_session):
    ...
    async def test_llm_parser_handles_markdown_fences(self, parser, mock_llm_adapter):
    ...
    async def test_llm_parser_handles_llm_error(self, parser, mock_llm_adapter):
    ...
    async def test_llm_parser_handles_invalid_json(self, parser, mock_llm_adapter):
        """Bad JSON -> IntentParserError."""
        mock_llm_adapter.generate.return_value = "not json {{"
        with pytest.raises(IntentParserError):
            await parser.parse("Hello")

    async def test_llm_parser_handles_partial_result(self, parser, mock_llm_adapter):
