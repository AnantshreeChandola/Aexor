...
class TestCreateSignerService:
    ...
    def test_factory_creates_service_with_valid_keys(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_factory_missing_private_key_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Missing PLAN_SIGNING_PRIVATE_KEY raises error."""
        monkeypatch.delenv("PLAN_SIGNING_PRIVATE_KEY", raising=False)
    ...
    def test_factory_missing_public_key_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Missing PLAN_SIGNING_PUBLIC_KEY raises error."""
        priv_hex, _ = _generate_hex_keys()
        monkeypatch.setenv("PLAN_SIGNING_PRIVATE_KEY", priv_hex)
        monkeypatch.delenv("PLAN_SIGNING_PUBLIC_KEY", raising=False)

        with pytest.raises(SigningKeyNotConfiguredError) as exc_info:
            create_signer_service()
        assert "PUBLIC_KEY" in exc_info.value.reason

    def test_factory_invalid_hex_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Non-hex string raises error."""
        monkeypatch.setenv("PLAN_SIGNING_PRIVATE_KEY", "not-valid-hex-string")
        monkeypatch.setenv("PLAN_SIGNING_PUBLIC_KEY", "also-not-hex")

        with pytest.raises(SigningKeyNotConfiguredError):
            create_signer_service()

    def test_factory_wrong_key_length_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
