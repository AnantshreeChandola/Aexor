...
class PreferenceService:
    ...
    async def health_check(self) -> dict:
        ...
        try:
            schema_count = len(self.schema_registry.list_preference_keys())
            health["schema_registry"] = f"healthy ({schema_count} schemas)"
        except Exception as e:
            health["schema_registry"] = f"error: {str(e)}"
        
        # Check encryption
        try:
            # Test encrypt/decrypt roundtrip
            test_data = "test"
            encrypted = self.encryption.encrypt_value(test_data)
            decrypted = self.encryption.decrypt_value(encrypted)
            if decrypted == test_data:
                health["encryption"] = "healthy"
            else:
                health["encryption"] = "unhealthy (roundtrip failed)"
        except Exception as e:
