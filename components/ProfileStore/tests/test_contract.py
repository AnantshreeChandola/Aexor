...
from components.ProfileStore.domain.models import (
    ConsentDeniedError, UserNotFoundError, UnknownPreferenceError
)
...
class TestGlobalSpecCompliance:
    ...
    def test_evidence_item_format_compliance(self):
        ...
        assert all(field in serialized for field in [
            "type", "key", "value", "confidence", "source_ref", "ttl_days", "tier"
        ])
...
class TestConsentEnforcement:
    ...
    async def test_get_all_preferences_consent_enforcement(self, service_with_consent_mocks):
        ...
        with pytest.raises(ConsentDeniedError):
            await service.get_all_preferences(
                user_id=user_id,
                context_tier=1
            )
